...
import static com.example.demo.util.LoggerUtils.getParameter;
...
public class LoggerInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object o) throws Exception {
        ...
        LoggerEntity logger = LoggerEntity.builder()
                .clientIp(getIpAddress(request))            //设置客户端ip
                .method(request.getMethod())                //设置请求方法
                .type(LoggerUtils.getRequestType(request))  //设置请求类型（json|普通请求）
                .paramData(getParameter(request))                       //设置请求参数内容json字符串
                .uri(url)                                   //设置请求地址
